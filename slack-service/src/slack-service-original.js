...
class SlackService {
  constructor(token, config) {
    ...
    this.client.on(WebClientEvent.RATE_LIMITED, (numSeconds) => {
      logger.warn(`Rate limited by Slack API, retry after ${numSeconds} seconds`);
    });
  ...
  async getUnrespondedMessages() {
    ...
    // 2. Remove duplicate messages (same text from same user within short time)
